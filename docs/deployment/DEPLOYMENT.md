...
## 8. D-line demo deployment notes

- Quick rehearsal uses `BLOCKCHAIN_AUDIT_MODE=local-mock`; do not present it as a real verifier result.
- Hardhat rehearsal requires `contract:node`, `contract:deploy`, `AUDIT_CONTRACT_ADDRESS`, and an API restart.
- Keep `docs/contracts/*.sample.json` available for offline fallback. These fixtures are safe to ship because they contain no private keys.
- Store screenshots and downloaded bundles under `docs/assets/demo-materials/`.
- For production, block the whole attack route family:
...
- If using SQLite, back up `VERIVOTE_SQLITE_PATH` before every demo reset.
